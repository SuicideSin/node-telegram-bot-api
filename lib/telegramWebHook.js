...
class TelegramBotWebHook {
  constructor(token, options, callback) {
    ...
    if (options.key && options.cert) { // HTTPS Server
      ...
      this._webServer = https.createServer(opts, (req, res) => this._requestListener(req, res));
    } else {
      debug('HTTP WebHook enabled');
      this._webServer = http.createServer((req, res) => this._requestListener(req, res));
    }
    ...
    this._requestListener = (req, res) => {
      ...
      if (!this.regex.test(req.url)) {
        ...
        req
          .pipe(bl((err, body) => this._parseBody(err, body)))
